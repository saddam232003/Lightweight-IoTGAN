...
# FULL CIFAR-10 Dataset Mode with Qualitative Results
# =============================================
...
# Qualitative Results
# ------------------
def save_qualitative_results(dataset, gen_model_path, out_path="qualitative_results.png", num_samples=9):
    ...
    with torch.no_grad():
        fake = gen(torch.randn(num_samples, Z_DIM, device=DEVICE))

    # Denormalize from [-1,1] to [0,1]
    real = (real * 0.5) + 0.5
    fake = (fake * 0.5) + 0.5

    # Make grid side-by-side
    grid = utils.make_grid(torch.cat((real, fake), dim=0), nrow=num_samples)
    utils.save_image(grid, out_path)
    print(f"✅ Saved qualitative results: {out_path}")
...
def train_gan(dataset, epochs=20, batch_size=64, z_dim=Z_DIM, lr=2e-4):
    ...
    save_qualitative_results(dataset, FP32_GEN_PATH)
...
print("✅ Full Training + INT8 Quantization + Qualitative Results Completed!")
